                    ...
                    <ul class="images cf">
                        <?php foreach($project->media()->limit(9)->toStructure() as $media): ?>
                            <li>
                                ...
                                <a href="<?php echo e($mediaType == 'video', $media->video(), c::get('pathThumbs') . '/' . $original->filename()) ?>" data-zoomable data-width="<?php echo $original->width() ?>" data-height="<?php echo $original->height() ?>"<?php if ($mediaType == 'video') : ?>data-type="video" data-video-fallback="<?php echo c::get('pathThumbs') . '/' . $screenshotName ?>.jpg"<?php endif ?>>
